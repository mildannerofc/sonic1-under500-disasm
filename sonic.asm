;  =========================================================================
; |              Sonic 1 Under 500 Disassembly by Mildanner                 |
;  =========================================================================
;
; Original disassembly created by Hivebrain
; Sonic 1 GitHub ASM68K Branch by Sonic Retro
...
; ===========================================================================
; ROM settings and adjustments
; ===========================================================================

ExtraMusic:		equ 1 ; 0 = No extra music 1 = Extra music

FollowPlayerDeath:	equ 0 ; 0 = Do not follow 1 = Follow (Credits: DeltaW)

SmoothSpecialRotation:	equ 1 ; 0 = original 1 = smooth

ProtoVictoryPose: equ 1 ; 0 = normal 1 = beta

LoadRingObject: equ 0 ; 0 = animals 1 = enemies

AltGotThrough:	equ 1 ; 0 = disabled 1 = enabled

WhiteFadeSega:	equ 1 ; 0 = disabled 1 = enabled

BetterFadeProgression:	equ 0	; 0 = standard 1 = improved

SonicBurnDeathSprite:	equ 0	; 0 = just die id sprite 1 = restores

BossHealthCount:   equ 8	; set boss hits count

SHCBuild:	equ 0	; 0 = deactivated SHC  1 = enabled SHC

DevBuild:	equ 0	; 0 = disabled, 1 = enabled

S1LevSelMus: equ 0	; 0 = music disabled 1 = music enabled

S2LevelSelect:	equ 1	; 0 = default s1 level select 1 = s2 level select by Esrael

S3KDebugHUD:	equ 0	; 0 = normal debug 1 = sonic 3 debug mode hud code 

HideHUDEnding:	equ 0	; 0 = hud in ending 1 = no hud

HUDScrolling:	equ 0	; 0 = stand HUD 1 = moving HUD

NameRebrand:	equ 1	; 0 = old 1 = new

DebugAltText:	equ 0	; 0 = same art load 1 = alternative art load HUD
...
loc_E0:
	...
	endc
		...
		dc.b "(C)MILD 2025.JUL" ; Copyright holder and release date (generally year)
		if NameRebrand = 1
		dc.b "Sonic 1 - Under 500 DX by Mildanner             " ; Domestic name
		dc.b "Sonic 1 - Under 500 DX by Mildanner             " ; International name
		else
		dc.b "SONIC 1 UNDER 500 BY MILDANNER                  " ; Domestic name
		dc.b "SONIC 1 UNDER 500 BY MILDANNER                  " ; International name
		endif
		if Revision=0
...
Art_Text:	incbin	"artunc\menutext.bin" ; text used in level select (S1)
		even
		if	DebugAltText=1
Art_DEBUGText:	incbin	"artunc\debugtext.bin" ; duplicate for debug mode
		even
		endif
; ===========================================================================
; ---------------------------------------------------------------------------
...
; End of function QuickPLC
		...
		if BetterFadeProgression=1
		include	"_inc\Palette Fading IMPROVED.asm"
		else
...
GM_Title:
		...
		if SHCBuild = 1
		jsr SHC       ; This code can be used if in SHC.
		endif
		if DevBuild=1
	...
	Tit_LoadText:
		...
		move.w	#$700,(v_demolength).w ; run title screen for $700 frames
...
LevSel_SndTest:
		...
		bhs.s	LevSel_Right
		move.w	#$FF,d0		; if sound test	moves below 0, set to $63
...
LevSel_Right:
		...
		moveq	#0,d0		; if sound test	moves above $63, set to	0
...
LevelMenuText:   if Revision=0
    dc.b    "LUCID HILLS        ACT 1"
        dc.b    "                   ACT 2"
        dc.b    "                   ACT 3"
        dc.b    "BLUE RUINS         ACT 1"
        dc.b    "                   ACT 2"
        dc.b    "                   ACT 3"
        dc.b    "SUNSET MARBLE      ACT 1"
        dc.b    "                   ACT 2"
        dc.b    "                   ACT 3"
        dc.b    "MIDNIGHT ROAD      ACT 1"
        dc.b    "                   ACT 2"
        dc.b    "                   ACT 3"
        dc.b    "CASINO YARD ZONE   ACT 1"
        dc.b    "                   ACT 2"
        dc.b    "                   ACT 3"
        dc.b    "EGG FORTRESS       ACT 1"
        dc.b    "                   ACT 2"
        dc.b    "                   ACT 3"
        dc.b    "FINAL ZONE              "            
        dc.b    "SPECIAL STAGE           "          
...
PlayCurrentActMusic:
		...
		bra.w	PlaySound		; play current level music
...
@normal:
		jsr	(PlayCurrentActMusic).l	; resume level music after drowning counting
